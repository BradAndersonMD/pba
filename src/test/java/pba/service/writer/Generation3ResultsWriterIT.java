...
class Generation3ResultsWriterIT {
  ...
  @Mock private PbaProperties pbaProperties;

  @InjectMocks private Generation3ResultsWriter resultsWriter;

  @Test
  void itShouldWriteResults() throws IOException {
    Generation3Results generation3Results = loadInput();
    when(pbaProperties.getOutputDirectory()).thenReturn(tempDir.toAbsolutePath().toString());
    assertDoesNotThrow(() -> resultsWriter.write(generation3Results));
  }

  private Generation3Results loadInput() throws IOException {
    URL input =
        Generation3ResultsWriterIT.class
            .getClassLoader()
            .getResource("writer/generation3/input.json");

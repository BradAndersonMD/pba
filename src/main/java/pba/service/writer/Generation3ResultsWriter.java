...
public class Generation3ResultsWriter implements ResultsWriter<Generation3Results> {
  ...
  public void write(Generation3Results results) {
    String fileName = pbaProperties.getOutputDirectory() + "/results.csv";
    try (FileWriter writer = new FileWriter(fileName)) {
      StatefulBeanToCsv<PokemonData> beanToCsv =
          new StatefulBeanToCsvBuilder<PokemonData>(writer).build();
